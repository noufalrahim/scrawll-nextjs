...
export default function DynamicForm({
  ...
  const applyConstraint = (value: string, constraint?: string) => {
    ...
    if (constraint === "emoji")
      return [...value].filter((v) => emojiRegex.test(v)).join("");
            ...
            >
              <div className="flex items-center gap-2">
                <div
                  ...
                  style={{ backgroundColor: v || "#000000" }}
                />
                <span>{v || field.placeholder || "Select color"}</span>
              </div>
            </Button>
          </PopoverTrigger>
          ...
          <PopoverContent className="w-[25rem] p-0">
            <Command>
              <CommandInput
                ...
                onValueChange={(val) => field.onSearch?.(val)}
              />
              <CommandList>
                {field.isLoading ? (
                  ...
                  {field.options?.map((opt) => (
                    ...
                    >
                      ...
                      <Check
                        className={cn(
                          "ml-auto",
                          autoValue === opt.value ? "opacity-100" : "opacity-0",
                        )}
                      />
                    </CommandItem>
                  ))}
