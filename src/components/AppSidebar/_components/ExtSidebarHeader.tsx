...
export default function ExtSidebarHeader() {
  ...
  const {
    ...
    `/workspaces?type=user-id&userId=${user?.id}`,
  );
      ...
      { name: data.name, createdBy: user.id },
      {
        onSuccess: (res) => {
          if (res?.success && res.data) {
            ...
            setOpen(false);
            refetchWorkspaces();
            localStorage.setItem("workspace", res.data.id!);
      ...
      { id: wId },
      {
        onSuccess: (res) => {
          if (res?.success) {
            ...
            toast.error("An error occured while deleting workspace", {
              position: "top-right",
            });
          }
        },
        ...
        <SidebarMenuItem>
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-[--radix-popper-anchor-width] p-1 rounded-lg shadow-lg border bg-white/95 backdrop-blur">
                  ...
                  >
                    <DropdownMenuItem
                      ...
                      onClick={() => {
                        ...
                        dispatch(setWorkspace(w as TWorkspace));
                      }}
                    >
                      {(w as TWorkspace).name}
                    </DropdownMenuItem>
                    ...
                    <DropdownMenu>
                      ...
                      >
                        <DropdownMenuItem onClick={() => setOpenMenu(true)}>
                          Settings
                        </DropdownMenuItem>
                        <DropdownMenuItem
              ...
              {workspaces && workspaces && workspaces?.data?.length > 0 && (
                <>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem
